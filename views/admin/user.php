...
<script>
    function Delete(idUser){
        ...
        if (!result) return 0;
        $.ajax({
            url: '<?= Url::to(['/admin/delete-user']) ?>?id='+idUser,
            type: 'DELETE',
    ...
    function Edit(idUser){
        $('#formUser').attr('action', '<?= Url::to(['/admin/edit-user']) ?>?id='+idUser);
        $('#_method').val("PATCH");
        $.ajax({
            url: '<?= Url::to(['/admin/view-one-user']) ?>?id='+idUser,
            type: 'GET',
            // data: { id: idUser },
            success: function(response) {
                if (response.error) {
    ...
    myModalEl.addEventListener('hidden.bs.modal', function (event) {
        $('#formUser').attr('action', '<?= Url::to(['/admin/add-user']) ?>');
        $('#_method').val("POST");
    })
</script>